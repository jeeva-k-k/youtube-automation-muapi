...
const SubtitleRenderer: React.FC<{ captions: CaptionPhrase[]; t: number }> = ({ captions, t }) => {
  ...
  return <div style={{ fontSize: '38px', fontWeight: 'bold', color: '#fff', textAlign: 'center', padding: '14px 30px', background: 'rgba(5,5,10,0.78)', borderRadius: '18px', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', textShadow: '0 2px 4px rgba(0,0,0,0.5)', maxWidth: '900px', lineHeight: '1.35' }}>{active.text}</div>;
};
...
const ProjectVisualRouter: React.FC<{ projectId: string; sceneId: string; title: string; eyebrow: string; theme: string }> = ({ projectId, sceneId, title, eyebrow, theme }) => {
  switch (projectId) {
    ...
    default: return <ChemosynthesisVisual sceneId={sceneId} title={title} eyebrow={eyebrow} theme={theme} />;
  }
};
...
const calculateMetadata: CalculateMetadataFunction<ProjectData> = ({ props }) => ({
  ...
  durationInFrames: Math.ceil((props.format?.duration || 10) * (props.format?.fps || 30))
});

export const MyComposition = () => {
  const inputProps = getInputProps() as ProjectData;
  const defaultProps: ProjectData = { project_id: 'chemosynthetic-vent', theme: 'botanical', title: 'DEEP OCEAN', eyebrow: 'GEOLOGY', format: { width: 1080, height: 1920, fps: 30, duration: 15 }, scenes: [{ id: 'intro', start: 0, end: 15, eyebrow: 'GEOLOGY', title: 'DEEP OCEAN', visual: { type: 'intro' } }], captions: [{ start: 0, end: 15, text: 'Test caption.' }], audioPath: '' };
