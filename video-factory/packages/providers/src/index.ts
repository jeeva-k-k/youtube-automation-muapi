...
// Interfaces mapping target workflow definitions
...
export class MuApiAdapter {
  ...
  private configPath = '/Users/jeeva/Documents/Codex/2026-07-16/https-muapi-ai-docs-mcp/.codex/config.toml';
  ...
  // ----------------------------------------------------
  public async generateTts(req: TtsRequest): Promise<TtsResult> {
    Logger.info('MuAPI TTS request started...', { textLength: req.text.length });
    const payload = {
      speakers: [{
  ...
  // ----------------------------------------------------
  public async uploadFile(req: StorageUploadRequest): Promise<StorageUploadResult> {
    Logger.info('Uploading file to MuAPI storage...', { fileName: req.fileName });
    const key = await this.getApiKey();
    const bytes = await readFile(req.filePath);
    ...
    Logger.info('Upload complete.', { url: body.url });
    return { url: body.url };
  }
  ...
  // Publishing Adapter
  // ----------------------------------------------------
  public async publishYoutube(req: PublishRequest): Promise<PublishResult> {
    Logger.info('Submitting YouTube publish request...', { title: req.title });
    const payload = {
    ...
    Logger.info('YouTube publish request submitted.', { requestId });
    return { requestId };
  }

  public async getPublishStatus(requestId: string): Promise<PublishStatusResult> {
    const body = await this.fetchApi(`/predictions/${requestId}/result`);
